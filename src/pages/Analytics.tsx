...
const Analytics: React.FC = () => {
  ...
  const handleRelapseSubmit = async () => {
    ...
    try {
      const result = await logRelapse(currentUser.uid, selectedTriggers, notes); // Pass triggers and notes
      if (result.success) {
                    ...
                    onValueChange={(newValue) => {
                      setSelectedTriggers((prevTriggers) => {
                        if (Array.isArray(newValue)) {
                          ...
                          const isAlreadySelected = prevTriggers.includes(newValue);
                    
                          if (isAlreadySelected) {
                            // Remove the value if it's already selected
                            return prevTriggers.filter((trigger) => trigger !== newValue);
                          } else {
                            // Add the value if it's not already selected
                            return [...prevTriggers, newValue];
                          }
                        }
                      });
                    }}
